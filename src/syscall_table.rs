...
impl SyscallTable {
    pub fn new(p: String) -> Result<SyscallTable, Box<dyn std::error::Error>> {
        ...
        let json_iter: Iter<'_, Value> = json["aaData"]
            .as_array()
            .unwrap()
            .iter();
        let _call_map: Option<HashMap<u64, String>> = json_iter.map(|item: &Value| -> Option<(u64, String)>{
                Some((
                    item.get(0)?.as_u64()?,
        ...
        Ok(SyscallTable { call_map, _args: args})
    }
}
